...
class ItemUpdateState(TypedDict):
...
class ItemUpdateGraph:
    ...
    def _validate_node(self, state):
        ...
        if not generated_code:
            ...
            try:
                ...
                field_id_set = set()
                for field in field_list:
                    field_id_set.add(field[FieldMeta.ID])

                del_field_id = []
                for field_id in field_value_map:
                    if field_id not in field_id_set:
                        del_field_id.append(field_id)

                for field_id in del_field_id:
                    del field_value_map[field_id]


                if not field_value_map:
                    user_feedback = interrupt(
                        f"Provide values for any fields to update"
                    )
                    self.retry_from_scratch = True
                    return {**state, GraphConstants.StateKeys.MESSAGES: [HumanMessage(content=user_feedback)]}
                else:
                    return {**state, GraphConstants.StateKeys.FIELD_VALUES_DICT: field_value_map}
    ...
    def _build_graph(self):
        ...
        """
        # Create the graph
        workflow = StateGraph(ItemUpdateState)
        ...
        workflow.add_node(GraphConstants.Nodes.UPDATE, self._update_node)
        ...
        workflow.add_conditional_edges(
            ...
            {"update": GraphConstants.Routes.UPDATE, GraphConstants.Routes.RETRY: GraphConstants.Routes.RETRY},
        )
        ...
        workflow.add_edge(GraphConstants.Nodes.UPDATE, GraphConstants.Routes.END)
